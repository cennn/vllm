...
def debug_attention(
        ...
        decode_query_pe: torch.Tensor,
        decode_query_nope: torch.Tensor,
        head_size: int,
        scale: float
):
    ...
    for b in range(batch_size):
        ...
        num_full_blocks = (end - start) - 1
        total_tokens = num_full_blocks * block_size + paged_kv_last_page_len[b]
    ...
    for b in range(batch_size):
        ...
        for i, block_idx in enumerate(block_indices_for_b):
            if i < (num_blocks_b - 1):
                ...
                if last_len > 0:
                    K_block = kv_cache[block_idx, 0][:last_len]
    ...
    k_pe_cache = V_out[:, :, 0, :head_size // 8]  # [bsz, kv_len, rope_head_dim]
    attn_weights_pe = torch.matmul(
        q_pe,
        k_pe_cache.transpose(1, 2)
    )

    q_nope = decode_query_nope  # [bsz, num_heads, latent_dim]
    compressed_kv_normed_cache = K_out.squeeze(2)  # [bsz, kv_len, latent_dim]
    attn_weights_nope = torch.matmul(
        q_nope,
        compressed_kv_normed_cache.transpose(1, 2)
    )

    attn_weights = (attn_weights_pe + attn_weights_nope) * scale
    attn_weights = torch.nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(q_nope.dtype)
    # [bsz,num_heads,kv_len] * [bsz,kv_len,latent_dim]
...
def debug_attention2(
    ...
    num_blocks, two_, block_size, num_kv_heads, nope_head_dim = kv_cache.shape
    assert two_ == 2, "kv_cache shape must be (num_blocks, 2, block_size, num_heads, head_dim)"
    ...
    # 计算最大键值长度
    b_req_idx = torch.arange(batch_size, device=device, dtype=int)
    b_seq_len = torch.empty((batch_size,), device=device, dtype=int)
    for b in range(batch_size):
        start = paged_kv_indptr[b]
        end = paged_kv_indptr[b + 1]
        num_full_blocks = (end - start) - 1  # 除最后一个块以外的块数
        total_tokens = num_full_blocks * block_size + paged_kv_last_page_len[b]
        b_seq_len[b] = total_tokens
    
    max_kv_len = max(b_seq_len).item()
    req_to_token = torch.zeros((batch_size, max_kv_len+1), device=device, dtype=int)
    
    # acc_seq_len, num_heads, head_dim
    rope_head_dim = decode_query_pe.shape[-1]
    head_dim = rope_head_dim + nope_head_dim
    k_buffer = torch.zeros((sum(b_seq_len) + len(b_seq_len)+1, num_kv_heads, head_dim), device=device, dtype=dtype)
    
    # acc_seq_len, num_heads, nope_head_dim
    v_buffer = torch.zeros((sum(b_seq_len) + len(b_seq_len)+1, num_kv_heads, nope_head_dim), device=device, dtype=dtype)
        
    copy_pos = 1
    for b in range(batch_size):
        req_to_token[b, :b_seq_len[b]+1] = torch.arange(copy_pos, copy_pos+b_seq_len[b]+1, device=device, dtype=int)
        start = paged_kv_indptr[b]
        end = paged_kv_indptr[b + 1]
        block_indices_for_b = paged_kv_indices[start:end]
        num_blocks_b = len(block_indices_for_b)
        for i, block_idx in enumerate(block_indices_for_b):
            if i < (num_blocks_b - 1):
                ...
                # K_out[b, copy_pos:copy_pos + block_size] = K_block
                # V_out[b, copy_pos:copy_pos + block_size] = V_block
                k_buffer[copy_pos:copy_pos + block_size,:, :rope_head_dim] = V_block[..., :rope_head_dim]
                k_buffer[copy_pos:copy_pos + block_size,:, rope_head_dim:] = K_block
                v_buffer[copy_pos:copy_pos + block_size] = K_block
                copy_pos += block_size
            else:
                last_len = paged_kv_last_page_len[b].item()
                if last_len > 0:
                    ...
                    # K_out[b, copy_pos:copy_pos + last_len] = K_block
                    # V_out[b, copy_pos:copy_pos + last_len] = V_block
                    k_buffer[copy_pos:copy_pos + last_len,:, :rope_head_dim] = V_block[..., :rope_head_dim]
                    k_buffer[copy_pos:copy_pos + last_len,:, rope_head_dim:] = K_block
                    v_buffer[copy_pos:copy_pos + last_len] = K_block
                copy_pos += last_len
        copy_pos += 1
    
    # batch, num_heads, head_dim(rope_head_dim+nope_head_dim)
    q = torch.cat((decode_query_pe,decode_query_nope), dim=-1)
    # batch, num_heads, nope_head_dim
    o = torch.zeros_like(decode_query_nope)
    
    num_kv_splits = 8
    num_q_heads = q.shape[1]
    attn_logits = torch.zeros((batch_size, num_q_heads, num_kv_splits, nope_head_dim+1), device=device, dtype=dtype)
